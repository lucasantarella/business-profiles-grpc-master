...
# generate the proto defintion file that we can pass to envoy so that knows the grpc service
# we want to expose
protoc -I./protos/ --include_imports \
                --include_source_info \
                --descriptor_set_out=profiles_service_definition.pb \
...
if [ "$(groups | grep -c docker)" -gt "0" ]; then
    ...
    docker run -it --rm -p 51051:51051 --name envoy --network="host" \
             -v "$(pwd)/profiles_service_definition.pb:/data/profiles_service_definition.pb:ro" \
             -v "$(pwd)/envoy-config.yml:/etc/envoy/envoy.yaml:ro" \
             envoyproxy/envoy
else
    echo "you are not in the docker group, running with sudo"
    echo "Envoy will run at port 51051 (see envoy-config.yml)"
    sudo docker run -it --rm -p 51051:51051 --name envoy \
             -v "$(pwd)/profiles_service_definition.pb:/data/profiles_service_definition.pb:ro" \
             -v "$(pwd)/envoy-config.yml:/etc/envoy/envoy.yaml:ro" \
             envoyproxy/envoy
fi
